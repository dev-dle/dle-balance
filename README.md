...
Данный раздел содержит настройки полей, без которых невозможна корректная работа модуля. Без указания вашего АПИ токена, поля в которое модуль будет проставлять ссылки на наши материалы и хотя бы одного из полей с ID kinopoisk, IMDB либо WorldArt  работа модуля будет невозможна. Перед началом работы с модулем убедитесь, что эти поля заполнены.
...
“Доп. поле для вставки первой озвучки” - в это поле модуль запишет только первую из доступных озвучек. Удобно, если выводите озвучку на главной сайта. Кроме того вы можете задать приоритет для первой озвучки, если какая-то из заданых озвучек доступна для видео, то в данное доп поле будет проставлена именно она, даже если в базе Collaps первая озвучка другая.
...
“Доп. поле для вставки Collaps id” - в это поле будет проставлен id  материала в базе Collaps. Доп поле обязательно к заполнению, если вы планируете использовать “Модуль календарь”.
...
В этом разделе у вас есть возможность сле
дить за обновлениями в базе Collaps и сразу добавлять к себе новости, которых еще нет у вас на сайте.
